								...
								if ($get_card_number_error != "") {
									...
									if (intval($card_duplicate["count(*)"]) == 0) {
										if ($_POST["checklive"] == "on") {
											$check = check($card_import["card_number"], $card_import["card_month"], $card_import["card_year"], $card_import["1"]);
										}
										else {
											$check = 1;
										}
									...
									<tr>
										...
										<td>
											<select class="card_value_editor" name="card_check">
												...
												<option value="<?=strval(CHECK_VALID)?>">VALID</option>
												<option value="<?=strval(CHECK_REFUND)?>">REFUNDED</option>
...
<?php
				switch ($value['card_check']) {
					...
					case strval(CHECK_INVALID):
						...
						break;
					case strval(CHECK_REFUND):
						echo "<span class=\"pink bold\">REFUNDED</span>";
						break;
					case strval(CHECK_UNKNOWN):
